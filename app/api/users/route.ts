...
export async function GET() {
  try {
    ...
    >(
      `SELECT u.id, u.business_id, u.name, u.email, u.role, u.pin, u.active, u.department, u.created_by, u.created_at,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, email, password, role, pin, department } = body;
    ...
    await execute(
      `INSERT INTO users (id, business_id, name, email, password_hash, role, pin, department, active, created_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        userId,
        session.user.businessId,
