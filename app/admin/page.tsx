...
const SECTION_ORDER: ActionSection[] = [
  "pos",
  "catalog",
  "inventory",
  "money",
...
export default function AdminDashboardPage() {
  ...
  const [salesByItemType, setSalesByItemType] = useState<
    { item_type: string; revenue: number; profit: number }[]
  >([]);
  ...
  const visibleButtons = ACTION_BUTTONS.filter((button) => {
    ...
    // For other roles (owner, admin), show all buttons without role restrictions
    return true;
  }).map((button) => {
  ...
  return (
    <AdminLayout>
                    ...
                    <div className="flex items-start gap-2.5 pr-6">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-semibold text-red-700 dark:text-red-300">
                          ...
                          {expiringBatches.expired.length !== 1
                            ? "es"
                            : ""}{" "}
                          expired
                        </p>
                    ...
                    <div className="flex items-start gap-2.5 pr-6">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-semibold text-amber-700 dark:text-amber-300">
                          ...
                          {expiringBatches.expiringSoon.length !== 1
                            ? "es"
                            : ""}{" "}
                          expiring soon
                        </p>
                          ...
                          {expiringBatches.expiringSoon
                            ...
                            .map((b) => {
                              const daysLeft = Math.ceil(
                                (b.expiry_date - Date.now() / 1000) / 86400,
                              );
            ...
            {statsLoading ? (
              ...
              <div className="rounded-xl sm:rounded-2xl border border-slate-200/80 dark:border-slate-700/80 bg-white dark:bg-[#1c2e18] shadow-sm overflow-hidden">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 divide-y sm:divide-y-0 sm:divide-x divide-slate-100 dark:divide-slate-800">
                  <Link
                    href="/admin/items"
                    className="p-3 sm:p-4 text-center hover:bg-slate-50/80 dark:hover:bg-[#1c6a1e]/5 transition-colors"
                  >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {salesByItemType.map((row) => {
                      const typeConfig = productTypes.find(
                        (t) => t.key === row.item_type,
                      );
                      const typeLabel = typeConfig
                        ? `${typeConfig.emoji} ${typeConfig.label}`
                        : row.item_type;
                      const totalRev = salesByItemType.reduce(
                        (s, r) => s + r.revenue,
                        0,
                      );
                      const pct =
                        totalRev > 0 ? (row.revenue / totalRev) * 100 : 0;
                      return (
                        <div
                          key={row.item_type}
                        ...
                        >
                          ...
                          <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">
                            {pct.toFixed(0)}% of revenue · KES{" "}
                            {Math.round(row.profit).toLocaleString()} profit
                          </p>
                        </div>
                      );
                    })}
      ...
      <Drawer
        open={categoryDrawerOpen && !isMobile}
      ...
      <Drawer
        open={stockAdjustDrawerOpen && !isMobile}
      ...
      <Drawer
        open={stockTakeDrawerOpen && !isMobile}
      ...
      >
        <DrawerContent className="!w-full sm:!w-[800px] md:!w-[900px] lg:!w-[1000px] !max-w-none h-full max-h-screen bg-white dark:bg-slate-900">
          ...
          <div className="overflow-y-auto px-4 sm:px-5 py-5 flex-1 bg-slate-50/50 dark:bg-slate-950/30">
            <StockTakeForm onCancel={() => setStockTakeDrawerOpen(false)} />
          </div>
        </DrawerContent>
      </Drawer>
      ...
      {/* Open Shift Drawer */}
      <Drawer
        open={openShiftDrawerOpen && !isMobile}
      ...
      {/* Close Shift Drawer */}
      <Drawer
        open={closeShiftDrawerOpen && !isMobile}
      ...
      <Drawer
        open={balanceApprovalsDrawerOpen && !isMobile}
      ...
      <Drawer
        open={withdrawalDrawerOpen && !isMobile}
      ...
      >
        <DrawerContent className="!w-full sm:!w-[420px] md:!w-[460px] !max-w-none h-full max-h-screen bg-white dark:bg-slate-900">
          ...
          <div className="overflow-y-auto flex-1 bg-slate-50/50 dark:bg-slate-950/30 px-4 sm:px-5 py-4">
            <div className="max-w-md mx-auto">
              <WithdrawalForm
                onSuccess={() => setWithdrawalDrawerOpen(false)}
              />
            </div>
          </div>
        </DrawerContent>
            ...
            <div className="space-y-6 max-w-2xl">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 pt-1">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">
                    Categories are broad groups that help organize what you
                    sell! Think of them like sections in a store - "Fruits" is a
                    category for all fruit items, "Vegetables" is a category for
                    all vegetable items, and "Snacks" is a category for all
                    snack items. This helps organize your products and makes
                    them easier to find!
                  </p>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 pt-1">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">
                    ...
                    (like "Mangoes"), set a price (how much money it costs), and
                    tell the system how many you have (like "50 pieces" or "20
                    kg").
                  </p>
                  <p className="text-xs text-slate-500 dark:text-slate-500 italic">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-800 dark:text-blue-200 mb-3">
                      ...
                      you can buy tomatoes in that aisle - by the kilogram, by
                      the piece, in different sizes, etc.!
                    </p>
                    <div className="bg-white dark:bg-slate-800 rounded p-3 text-xs text-blue-900 dark:text-blue-100">
                      ...
                      <p className="mt-2 text-blue-700 dark:text-blue-300">
                        When you tap "Tomatoes" on the POS screen (the checkout
                        counter), you'll see all the different ways to sell
                        tomatoes and pick the right one!
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 pt-1">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">
                    When adding a new item, choose "Parent Item" mode. Give it a
                    name like "Tomatoes" or "Eggs". Parent items don't have
                    prices or stock - they're just containers!
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 pt-1">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">
                    After creating a parent, add variants! Click "Add Item" and
                    choose "Variant" mode. Select the parent (like "Tomatoes")
                    and give it a variant name (like "Per Kg" or "Big Size").
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 pt-1">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">
                    Click the big green "Open POS" button at the top! This opens
                    your checkout screen - like the cashier's computer at a
                    supermarket. When a customer brings items to you, you tap on
                    the products on the screen to add them to their bill, then
                    take their payment. It's your digital cash register!
                  </p>
                  <p className="text-xs text-slate-500 dark:text-slate-500 italic">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 pt-1">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">
                    Click "View Stock" to see how many of each product you have.
                    The system automatically reduces stock when you sell
                    something, so you always know what's left!
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 pt-1">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">
                    ...
                    "Stock Adjustment" to fix the numbers. Tell the system what
                    happened and how many items to add or remove.
                  </p>
                  <p className="text-xs text-slate-500 dark:text-slate-500 italic">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 pt-1">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">
                    Sometimes you need to count everything you have in your
                    shop! Use "Stock Take" to count all your products and update
                    the numbers in the system. This helps keep everything
                    accurate!
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-emerald-800 dark:text-emerald-200 mb-3">
                      The Profit page is like a magic calculator that tells you
                      if you're making money! It shows you three important
                      numbers:
                    </p>
                    <div className="bg-white dark:bg-slate-800 rounded p-3 text-xs space-y-2">
                      ...
                      <div>
                        ...
                        <p className="text-emerald-700 dark:text-emerald-300">
                          This is all the money you spent to buy those things
                          from suppliers!
                        </p>
                      </div>
                      <div>
                        ...
                        <p className="text-emerald-700 dark:text-emerald-300">
                          This is what's left! Sales minus Costs = Your Profit
                          (the money you actually made!)
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 pt-1">
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">
                    Click "View Profit" to see your money! You can choose to see
                    profits for "Today", "This Week", "This Month", or pick your
                    own dates. The page shows you which products made the most
                    money, so you know what to sell more of!
                  </p>
                  <p className="text-xs text-slate-500 dark:text-slate-500 italic">
                    Example: If you see "Mangoes" made KES 5000 profit, that
                    means selling mangoes was really good for your business!
                  </p>
                </div>
              </div>
              ...
              <div className="bg-yellow-50 dark:bg-yellow-950/20 border border-yellow-200 dark:border-yellow-900 rounded-lg p-4">
                <ul className="space-y-2 text-sm text-yellow-900 dark:text-yellow-100">
                  ...
                  <li className="flex items-start gap-2">
                    <span className="font-bold">•</span>
                    <span>
                      If stock numbers seem wrong, use Add Stock or Stock Take
                      to fix them!
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
