...
export default function RetailProfitPage() {
  ...
  async function fetchProfitData() {
    try {
      ...
      const res = await fetch(`/api/profit?start=${startTs}&end=${endTs}&itemType=retail`);
      ...
      <AdminLayout>
        <div className="flex items-center justify-center h-screen">
          <div className="text-center space-y-3">
            ...
            <p className="text-slate-500 text-sm">Loading retail profit...</p>
          </div>
        </div>
      </AdminLayout>
    );
  ...
  return (
    <AdminLayout>
      <div className="min-h-screen">
        {/* ─── Header ─────────────────────────────────────────────── */}
        <div className="sticky top-0 z-10 bg-white/95 dark:bg-[#0f1a0d]/95 backdrop-blur-lg border-b-2 border-blue-100 dark:border-blue-900/50">
          <div className="px-4 md:px-6 py-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-lg md:text-xl font-black text-slate-900 dark:text-white">Retail Items</h1>
        ...
        <div className="p-4 md:p-6 space-y-5 max-w-7xl mx-auto pb-24 md:pb-6">
          {/* ─── Date Filter ──────────────────────────────────────── */}
          <div className="flex flex-wrap items-center gap-3 pb-4 border-b-2 border-blue-100 dark:border-blue-900/30">
            <div className="flex items-center gap-1 p-1 bg-blue-50 dark:bg-blue-900/20 border-2 border-blue-200 dark:border-blue-800 rounded-lg">
          ...
          <div className="grid grid-cols-3 md:grid-cols-6 gap-2">
            <div className="border-2 border-blue-600 bg-blue-600 p-3 text-center rounded-lg">
            ...
            <div className="border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg">
              <div className="p-4 border-b-2 border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="outline" className="text-[10px] border-blue-300 text-blue-600">{topItems.length}</Badge>
                </div>
              </div>
              <div className="p-3 space-y-2">
                {topItems.length === 0 ? (
                  ...
                  return (
                    <div key={item.item_id} className="p-2.5 border-2 border-blue-100 dark:border-blue-900/30 bg-blue-50/50 dark:bg-blue-900/10 rounded-lg">
                      <div className="flex items-center gap-2 mb-1.5">
                        <div className="w-6 h-6 flex items-center justify-center border-2 border-blue-600 bg-blue-600 rounded">
                        ...
                        <p className="text-xs font-black text-blue-600">+{formatPrice(item.total_profit)}</p>
                      </div>
                      <div className="h-1.5 bg-blue-100 dark:bg-blue-900/30 rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-blue-400 to-indigo-500 rounded-full transition-all" style={{ width: `${barWidth}%` }} />
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="p-3 space-y-2">
                {lossItems.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <p className="text-blue-600 text-sm font-bold">All items profitable!</p>
                  </div>
          ...
          <div>
            <h2 className="text-xs font-black text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-2">
              <BarChart3 className="w-3.5 h-3.5" /> Retail Profit Calendar
            </h2>
            <ProfitCalendar itemType="retail" />
          </div>
          ...
          <div className="border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg">
            <div className="p-4 border-b-2 border-slate-200 dark:border-slate-700">
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-black text-sm text-slate-900 dark:text-white">All Retail Items</h3>
            ...
            {profitData.itemProfits.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-slate-500 text-sm">No retail sales in this period</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-blue-50 dark:bg-blue-900/10 border-b-2 border-slate-200 dark:border-slate-700">
                        ...
                        return (
                          <tr key={item.item_id} className={`border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/30 ${!hasBuyPrice ? 'bg-yellow-50/50 dark:bg-yellow-900/5' : ''}`}>
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                {isPositive ? <TrendingUp className="h-3.5 w-3.5 text-blue-500 flex-shrink-0" /> : <TrendingDown className="h-3.5 w-3.5 text-red-500 flex-shrink-0" />}
                            ...
                            <td className="px-4 py-3 text-right font-semibold text-slate-600 dark:text-slate-300 text-xs">{item.quantity_sold.toFixed(0)}</td>
                            ...
                            <td className={`px-4 py-3 text-right font-bold text-xs ${isPositive ? 'text-blue-600' : 'text-red-500'}`}>{formatPrice(avgProfit)}</td>
                            <td className={`px-4 py-3 text-right font-black text-xs ${isPositive ? 'text-blue-600' : 'text-red-500'}`}>{formatPrice(item.total_profit)}</td>
                            <td className="px-4 py-3 text-right">
                              <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${isPositive ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' : 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'}`}>
                ...
                {!showAllItems && profitData.itemProfits.filter(i => i.item_name.toLowerCase().includes(itemSearch.toLowerCase())).length > 15 && (
                  <div className="p-3 text-center border-t border-slate-100 dark:border-slate-800">
                    <button onClick={() => setShowAllItems(true)} className="text-xs font-bold text-blue-600 hover:text-blue-700">
