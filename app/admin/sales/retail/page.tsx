...
export default function RetailSalesPage() {
  const [data, setData] = useState<RetailAnalyticsData | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      let url = `/api/sales/analytics/daily?itemType=retail&days=${days}`;
      if (selectedDate) url += `&date=${selectedDate}`;
      const result = await apiFetch<RetailAnalyticsData>(url, { cache: 'no-store' });
      if (result.success && result.data) {
        setData(result.data);
      } else {
        setError(result.message || 'Failed to load retail data');
      }
    } catch {
      setError('Failed to load retail sales data');
    } finally {
      setLoading(false);
    }
  }, [days, selectedDate]);
  ...
  const fetchDayProducts = useCallback(async (dateKey: string) => {
    ...
    try {
      ...
      const result = await apiFetch<RetailAnalyticsData>(
        `/api/sales/analytics/daily?itemType=retail&days=1&date=${dateKey}`,
        { cache: 'no-store' }
      );
      if (result.success && result.data) {
      ...
      <AdminLayout>
        <div className="flex items-center justify-center h-screen">
          <div className="text-center space-y-4">
            ...
            <p className="text-slate-500 dark:text-slate-400">Loading retail analytics...</p>
          </div>
        </div>
      </AdminLayout>
    );
  ...
  return (
    <AdminLayout>
      <div className="min-h-screen">
        {/* Header */}
        <div className="sticky top-0 z-10 bg-white/95 dark:bg-[#0f1a0d]/95 backdrop-blur-lg border-b-2 border-blue-100 dark:border-blue-900">
          <div className="px-4 md:px-6 py-4">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div className="w-10 h-10 bg-gradient-to-br from-blue-400 to-indigo-600 flex items-center justify-center rounded-lg shadow-lg shadow-blue-500/20">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {[
                ...
                <Button
                  ...
                  className={`h-8 text-xs ${days === opt.value ? 'bg-blue-600 hover:bg-blue-700' : ''}`}
                >
        ...
        <div className="p-4 md:p-6 pb-24 md:pb-6 max-w-6xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="bg-gradient-to-br from-blue-500 to-indigo-600 border-0 shadow-lg shadow-blue-500/20">
              <CardContent className="p-4">
                ...
                <p className="text-blue-100 text-xs font-medium mb-1">Today&apos;s Revenue</p>
                <p className="text-2xl font-black text-white">{formatPrice(comparison.todayRevenue)}</p>
                <p className="text-[10px] text-blue-200 mt-1">Yesterday: {formatPrice(comparison.yesterdayRevenue)}</p>
              </CardContent>
            </Card>
            ...
            <Card className="bg-gradient-to-br from-emerald-500 to-green-600 border-0 shadow-lg">
              <CardContent className="p-4">
                ...
                <p className="text-emerald-100 text-xs font-medium mb-1">Today&apos;s Profit</p>
                <p className="text-2xl font-black text-white">{formatPrice(comparison.todayProfit)}</p>
                <p className="text-[10px] text-emerald-200 mt-1">Margin: {comparison.todayRevenue > 0 ? ((comparison.todayProfit / comparison.todayRevenue) * 100).toFixed(1) : 0}%</p>
              </CardContent>
            </Card>
              ...
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-black text-blue-600">{formatPrice(summary.totalProfit)}</p>
                  </div>
                  <div className="w-px h-10 bg-slate-200 dark:bg-slate-700" />
                  <div>
          ...
          <Card className="border-2 border-slate-200 dark:border-slate-700">
            ...
            <CardContent className="space-y-2">
              {dailySales.length === 0 ? (
                <p className="text-sm text-slate-400 py-8 text-center">No retail sales data for this period</p>
              ) : (
                dailySales.map((day) => {
                        ...
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center gap-4">
                            <div className="text-right">
                              <p className="font-black text-base text-blue-600">{formatPrice(day.total_revenue)}</p>
                                ...
                                {products.map((p) => (
                                  <div key={p.item_id} className="grid grid-cols-12 gap-2 items-center px-2 py-1.5 rounded-lg hover:bg-white dark:hover:bg-slate-800 text-sm">
                                    ...
                                    <div className="col-span-2 text-right">
                                      ...
                                      <span className="text-[10px] text-slate-400 ml-0.5">pcs</span>
                                    </div>
                                    <div className="col-span-2 text-right font-bold text-xs text-blue-600">
            ...
            <Card className="border-2 border-slate-200 dark:border-slate-700">
              <CardHeader className="pb-3">
                <CardTitle className="text-base font-bold flex items-center justify-between">
                  ...
                  {bestHour && (
                    <Badge className="bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300 border-0 text-[10px]">
                          ...
                          <div
                            className={`absolute bottom-0 left-0 right-0 rounded-t-sm transition-all duration-300 ${
                              isPeak
                                ...
                                : 'bg-gradient-to-t from-blue-200 to-blue-100 dark:from-blue-800 dark:to-blue-700'
                            }`}
                    ...
                    <div key={cat.category_name} className="space-y-1.5">
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <div className="flex items-center gap-4 text-xs">
                          ...
                          <span className="font-black text-blue-600 min-w-[80px] text-right">{formatPrice(cat.total_revenue)}</span>
                        </div>
                      </div>
                      <div className="relative h-2.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden">
                        <div
                          className="absolute inset-y-0 left-0 bg-gradient-to-r from-blue-400 to-indigo-500 rounded-full transition-all duration-700"
                          style={{ width: `${barWidth}%` }}
                        />
                      </div>
              ...
              {filteredProducts.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <p className="text-sm text-slate-400">
                    {searchQuery ? 'No products match your search' : 'No retail products sold today'}
                  </p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                          ...
                          <tr key={p.item_id} className={`border-b border-slate-50 dark:border-slate-800/50 ${idx % 2 === 0 ? '' : 'bg-slate-50/30 dark:bg-slate-800/20'}`}>
                            <td className="px-4 py-2.5">
                              <div className="flex items-center gap-2">
                                <div className="w-7 h-7 rounded-lg bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center text-[10px] font-black text-blue-600">
                                  {idx + 1}
                                </div>
                            ...
                            <td className="px-4 py-2.5 text-right">
                              <span className="font-bold text-sm text-blue-600">{formatPrice(p.total_revenue)}</span>
                            </td>
                            <td className="px-4 py-2.5 text-right">
                            ...
                            <td className="px-4 py-2.5 text-right text-sm text-slate-500">
                              {formatPrice(p.avg_sell_price)}/pc
                            </td>
                            <td className="px-4 py-2.5 text-right">
                              {isOutOfStock ? (
                  ...
                  {/* Totals Row */}
                  <div className="flex items-center justify-between px-4 py-3 bg-blue-50 dark:bg-blue-900/20 border-t-2 border-blue-200 dark:border-blue-800">
                    <span className="font-bold text-sm text-blue-700 dark:text-blue-300">
                      Total ({filteredProducts.length} products)
                    </span>
                    <div className="flex items-center gap-6">
                      <div className="text-right">
                        <p className="text-[10px] text-blue-600 font-medium">Revenue</p>
                      ...
                      <div className="text-right">
                        <p className="text-[10px] text-blue-600 font-medium">Profit</p>
                        <p className="font-black text-sm text-blue-700 dark:text-blue-300">
                          ...
                          <div className="mt-2 pt-2 border-t border-slate-200 dark:border-slate-700 space-y-1">
                            <div className="flex justify-between text-xs">
                              ...
                              <span className="font-bold text-slate-900 dark:text-white">
                                {formatNumber(item.total_quantity_sold)} pcs
                              </span>
                            </div>
                            <div className="flex justify-between text-xs">
                              <span className="text-slate-500">Revenue</span>
                              <span className="font-bold text-blue-600">{formatPrice(item.total_revenue)}</span>
                            </div>
                            <div className="flex justify-between text-xs">
            ...
            <Card className="border-2 border-orange-200 dark:border-orange-800">
              <CardHeader className="pb-3">
                <CardTitle className="text-base font-bold flex items-center gap-2 text-orange-600">
                  ...
                  Retail Stock Alerts ({stockAlerts.length})
                </CardTitle>
              </CardHeader>
              <CardContent>
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className="text-[10px] text-slate-500">
                              Stock: {formatNumber(alert.current_stock)} pcs
