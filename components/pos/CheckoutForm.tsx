...
import { Loader2, Smartphone, CheckCircle2, XCircle, ChevronDown, Wallet, Receipt } from 'lucide-react';
...
export function CheckoutForm({ onBackToCart, onContinueShopping, onSaleComplete }: CheckoutFormProps = {}) {
  ...
  const [walletCreditAccountId, setWalletCreditAccountId] = useState<string | null>(null);
  const [walletAmountApplied, setWalletAmountApplied] = useState(0);
  const [creditWalletBalance, setCreditWalletBalance] = useState<number | null>(null);
  ...
  const [checkoutWalletBalanceLoading, setCheckoutWalletBalanceLoading] = useState(false);

  const MAX_POLL_COUNT = 60;
  const POLL_INTERVAL = 3000;

  const EPS = 0.01;
  const amountDue = Math.max(0, Math.round((total - walletAmountApplied) * 100) / 100);
  const walletCoversFull = total > EPS && walletAmountApplied + EPS >= total;

  const cashAmount = parseFloat(cashReceived) || 0;
  const cashExcessToWallet =
    paymentMethod === 'cash' && walletCreditAccountId && cashAmount > amountDue + EPS
  ...
  useEffect(() => {
    ...
    const enteredCredit = prev != null && prev !== 'credit' && paymentMethod === 'credit';
  ...
  useEffect(() => {
    ...
    const accountIdForWallet =
      paymentMethod === 'credit'
        ? creditAccountId
        : paymentMethod === 'cash' || paymentMethod === 'mpesa' || paymentMethod === 'split'
    ...
    (async () => {
      const res = await apiGet<{ account: CreditAccount | null; transactions: unknown[] }>(
  ...
  useEffect(() => {
    if (paymentMethod !== 'credit' || !creditAccountId || creditWalletBalance === null) return;
    const max = Math.round(Math.min(creditWalletBalance, total) * 100) / 100;
  ...
  const initiateMpesaPayment = async () => {
    if (amountDue < EPS) {
      setError('Nothing to collect by M-Pesa — wallet covers this sale. Use Mark Paid.');
      return;
    }
    ...
    try {
      if (!isOnline) {
        ...
        if (walletAmountApplied > EPS || walletCreditAccountId) {
          setError('Store wallet needs a connection. Go online to use wallet or cash overpayment to wallet.');
          setIsProcessing(false);
          return;
        }
    ...
    return (
      <div className="flex flex-col items-center justify-center flex-1 p-6">
        <div className="w-full max-w-sm text-center space-y-5">
          ...
          <div className="text-2xl font-bold text-[#1c6a1e]">{formatPrice(amountDue)}</div>
            ...
            <div className="flex items-center gap-2">
              <div className="flex flex-col items-end gap-0.5">
                {walletAmountApplied > EPS ? (
                  <>
                    <span className="text-xs text-slate-400 line-through">{formatPrice(total)}</span>
                    <span className="text-base font-bold text-[#1c6a1e]">{formatPrice(amountDue)} due</span>
                  </>
                ) : (
        ...
        <div className="p-4 space-y-4">
          <div>
            ...
            {(paymentMethod === 'credit' && creditAccountId) || walletCreditAccountId ? (
              <div className="mt-2.5 pt-2.5 border-t border-slate-200 dark:border-slate-700 space-y-1">
                ...
                {!isOnline ? (
                  <p className="text-xs text-slate-500 dark:text-slate-400">Connect to view wallet balance</p>
                ) : checkoutWalletBalanceLoading ? (
                  ...
                  <p className="text-sm text-slate-700 dark:text-slate-200">
                    <span className="text-slate-500 dark:text-slate-400">Store wallet balance</span>{' '}
          ...
          {paymentMethod === 'cash' && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 space-y-3 ring-1 ring-slate-200 dark:ring-slate-800">
              ...
              {cashReceived && (
                <div className="space-y-2">
                  {walletCreditAccountId && cashExcessToWallet > EPS && cashAmount + EPS >= amountDue && (
                    <p className="text-xs text-violet-700 dark:text-violet-300 font-medium px-1">
                      {formatPrice(cashExcessToWallet)} will be added to this customer&apos;s wallet (no cash change).
                    </p>
                  )}
                  <div
                  ...
                  >
                    ...
                    >
                      {formatPrice(
                        cashAmount + EPS >= amountDue ? Math.abs(change) : Math.abs(amountDue - cashAmount)
                      )}
                    </span>
                  </div>
                </div>
                  ...
                  <p className="text-lg font-bold text-orange-700 dark:text-orange-300 mt-2">
                    ...
                    {walletAmountApplied > EPS && (
                      <span className="block text-xs font-normal text-orange-600/90">
                        After wallet ({formatPrice(walletAmountApplied)} applied)
                      </span>
                    )}
                  </p>
                </div>
          ...
          {paymentMethod === 'credit' && (
            <>
              ...
              />
              {creditAccountId && creditWalletBalance != null && creditWalletBalance > EPS && (
                <div className="rounded-2xl p-4 space-y-3 ring-1 ring-violet-200 dark:ring-violet-900/50 bg-violet-50/40 dark:bg-violet-950/20">
                  <Label htmlFor="creditWalletAmt" className="text-sm font-medium text-violet-900 dark:text-violet-100">
                    Pay from wallet · max {formatPrice(Math.min(creditWalletBalance, total))} (default)
                  </Label>
                  <div className="flex gap-2">
                    <Input
                      ...
                      onChange={(e) => setCreditWalletAmountInput(e.target.value)}
                      onBlur={() => {
                        const n = Math.round((parseFloat(creditWalletAmountInput) || 0) * 100) / 100;
                        const max = Math.min(creditWalletBalance, total);
                        const capped = Math.max(0, Math.min(n, max));
                        setWalletAmountApplied(capped);
                        setCreditWalletAmountInput(capped > 0 ? String(capped) : '');
                      }}
                      placeholder="0"
                      className="h-11"
                      disabled={!isOnline}
                    ...
                    <Button
                      ...
                      onClick={() => {
                        const max = Math.min(creditWalletBalance, total);
                        setWalletAmountApplied(max);
                        setCreditWalletAmountInput(max > 0 ? String(max) : '');
                      }}
                    >
                      Max
                    </Button>
                  ...
                  {!isOnline && (
                    <p className="text-xs text-amber-600 dark:text-amber-400">Wallet needs a connection.</p>
                  )}
                </div>
              )}
            </>
        ...
        {paymentMethod && items.length > 0 && (
          ...
          >
            ...
            <div className="px-3.5 py-3 space-y-2.5 text-sm">
              ...
              {walletAmountApplied > EPS && (
                <div className="flex justify-between items-center gap-2 rounded-lg bg-violet-50 dark:bg-violet-950/35 px-2.5 py-2 ring-1 ring-violet-200/80 dark:ring-violet-800/50">
                  <span className="flex items-center gap-2 text-violet-800 dark:text-violet-200 min-w-0">
                    <Wallet className="h-4 w-4 shrink-0 text-violet-600 dark:text-violet-400" aria-hidden />
                    <span className="leading-tight">
                      <span className="block font-medium">From store wallet</span>
              ...
              {paymentMethod === 'cash' && walletCreditAccountId && cashExcessToWallet > EPS && (
                <div className="flex justify-between items-center gap-2 rounded-lg bg-emerald-50/80 dark:bg-emerald-950/25 px-2.5 py-2 ring-1 ring-emerald-200/70 dark:ring-emerald-900/40">
                  <span className="flex items-center gap-2 text-emerald-900 dark:text-emerald-100 min-w-0">
                    ...
                    <span className="leading-tight text-[13px]">
                      <span className="block font-medium">Change → wallet</span>
              ...
              <div className="flex justify-between items-start gap-2">
                <div className="min-w-0">
                  ...
                  {paymentMethod === 'credit' && amountDue < EPS && walletAmountApplied > EPS && (
                ...
                <div className="pt-2 mt-0.5 border-t border-dashed border-slate-200 dark:border-slate-600">
                  <div className="flex justify-between items-center gap-2 text-[13px]">
                    <span className="flex items-center gap-1.5 text-slate-600 dark:text-slate-400">
                      <Wallet className="h-3.5 w-3.5 text-violet-500 dark:text-violet-400 shrink-0" aria-hidden />
                      <span className="leading-tight">
          ...
          <>
            <div className="grid grid-cols-2 gap-2">
              <Button
                ...
                disabled={isProcessing || isMpesaInitiating || !isOnline || amountDue < EPS}
                title={
                  !isOnline
                    ? 'Requires connection'
                    : amountDue < EPS
            ...
            <p className="text-[10px] text-center text-slate-500 dark:text-slate-400 leading-snug">
              {isOnline ? 'Online Pay opens M-Pesa · Mark Paid if they paid outside the prompt' : 'Offline: use Mark Paid only'}
            </p>
          </>
        )}
            ...
            {isProcessing ? (
              ...
              <>
                <span className="flex items-center justify-center gap-2">
                  <CheckCircle2 className="h-5 w-5 opacity-90 shrink-0" aria-hidden />
