...
export async function seedDatabase() {
  try {
    ...
    // 2.5. Create a department staff user
    const deptStaffId = generateUUID();
    console.log("Creating department staff user...");
    await execute(
      `INSERT INTO users (id, business_id, name, email, password_hash, role, pin, department, active, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        deptStaffId,
        businessId,
    ...
    console.log(`✓ Created department staff user: ${deptStaffId}`);
    ...
    for (const item of items) {
      ...
      console.log(
        `✓ Created item: ${item.name} (${item.unitType}) - KES ${item.price}`,
      );
    }
