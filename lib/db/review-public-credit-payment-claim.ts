...
type ReviewResult =
  ...
  | { ok: false; code: 'not_found' | 'bad_state' | 'conflict' | 'shift_required'; message: string };
...
export async function approvePublicCreditPaymentClaim(
  ...
  if (!row || row.type !== 'payment') {
    return { ok: false, code: 'not_found', message: 'Payment record not found' };
  }
  if (row.public_claim_status !== 'pending') {
    return { ok: false, code: 'bad_state', message: 'This claim is not awaiting approval' };
  }

  const account = await queryOne<{ total_credit: number; customer_name: string }>(
